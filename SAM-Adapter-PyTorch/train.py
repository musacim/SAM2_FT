...
def make_data_loader(spec, tag=''):
    ...
    loader = DataLoader(dataset, batch_size=spec['batch_size'],
        shuffle=(sampler is None), num_workers=8, pin_memory=True, sampler=sampler,drop_last=True)
    return loader
...
def prepare_training():
    ...
    lr_scheduler = CosineAnnealingLR(optimizer, max_epoch, eta_min=float(config.get('lr_min', 0)) )
    if local_rank == 0:
...
def main(config_, save_path, args):
    ...
    lr_scheduler = CosineAnnealingLR(model.optimizer, config['epoch_max'], eta_min=float(config.get('lr_min', 0)) )
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        model = model.module  # unwrap for direct access
    else:
        model = model.cuda()  # Just use single-GPU mode
